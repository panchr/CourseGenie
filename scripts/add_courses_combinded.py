...
def main():
	...
	for record in raw_data:
		...
		try:
			course = Course.objects.get(name=name, id_number=id_number, number=number, department=department, letter=letter)
			if course.term != Course.TERM_INCONSISTENT and course.term != CURRENT_TERM:
				course.term = Course.TERM_BOTH
				course.save()
		except Course.DoesNotExist:
			course = Course(name=name, number=number, id_number=id_number, department=department, letter=letter, term=CURRENT_TERM)
			course.save()
			...
			if "crosslistings" in record:
				...
				for item in record["crosslistings"]:
					...
					crosslisting = CrossListing(course=course, number=number, id_number=id_number, letter=letter, department=department)
	...
	with open("data/courses.json") as f:
		...
		for record in raw_data:
			try: 
				course = Course.object.get(course_id=record["courseid"])
